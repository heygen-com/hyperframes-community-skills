#!/usr/bin/env node
// seam-gate.mjs — numeric seam verification gate verifier for HyperFrames films (motion-continuity).
...
if (!["verify", "probe"].includes(mode)) {
  console.error("usage: seam-gate.mjs verify --ledger ledger.json (--project <dir> | --url <preview-url>) [--json]");
  console.error("       seam-gate.mjs probe --t <seconds> (--project <dir> | --url <preview-url>) [--window 0.1]");
  process.exit(2);
}
...
const HYPERFRAMES_VERSION = "0.8.14";
...
function previewEnv() {
  const allowed = ["PATH", "HOME", "USER", "TMPDIR", "TMP", "TEMP", "SystemRoot", "WINDIR",
    "LOCALAPPDATA", "APPDATA", "XDG_CACHE_HOME", "XDG_CONFIG_HOME", "SHELL", "LANG", "LC_ALL"];
  const env = {};
...
async function httpOk(url) {
  try { const r = await fetch(url, { signal: AbortSignal.timeout(2000) }); return r.ok; } catch { return false; }
}
...
function localUrl(value, label) {
  ...
  if (!["http:", "https:"].includes(parsed.protocol) || !localHosts.has(parsed.hostname))
    throw new Error(`${label} must use http(s) on localhost`);
...
async function ensureServer() {
  ...
  if (!base) {
    const projectArg = flag("project", null);
    if (!projectArg) throw new Error("need --url or --project");
    ...
    const child = spawn(npx, ["--yes", `hyperframes@${HYPERFRAMES_VERSION}`, "preview", "--no-open",
      "--force-new", "--foreground", "--port", String(port)],
      { cwd: project, env, stdio: ["ignore", "pipe", "pipe"], detached: true });
    let serverError = "";
    child.stdout.on("data", (data) => { serverError += data; });
    child.stderr.on("data", (data) => { serverError += data; });
    ...
    while (Date.now() < deadline) {
      ...
      if (child.exitCode !== null)
        throw new Error(`preview server exited early (${child.exitCode}): ${serverError.slice(-500)}`);
      await new Promise((r) => setTimeout(r, 500));
    }
    if (!(await httpOk(base + "/api/projects"))) throw new Error("preview server never became ready");
  }
  base = localUrl(base, "--url");
  ...
  if (!compUrl) {
    ...
    compUrl = `${base}/api/projects/${encodeURIComponent(id)}/preview/comp/index.html`;
  }
  return localUrl(compUrl, "--comp-url");
}
    ...
    for (const v of versions) {
      ...
      for (const plat of readdirSync(vdir)) {
        const bin = kind === "chrome-headless-shell"
          ...
          : join(vdir, plat, "Google Chrome for Testing.app", "Contents", "MacOS", "Google Chrome for Testing");
...
async function launchChrome() {
  ...
  const profile = mkdtempSync(join(tmpdir(), "vox-seam-chrome-"));
  const args = ["--remote-debugging-port=0", "--no-first-run", "--no-default-browser-check",
    "--mute-audio", "--hide-scrollbars", "--disable-extensions", `--user-data-dir=${profile}`,
...
class CDP {
  ...
  send(method, params = {}, sessionId, timeoutMs = 30_000) {
    ...
    return new Promise((res, rej) => {
      ...
      setTimeout(() => { if (this.pending.has(id)) { this.pending.delete(id); rej(new Error(method + " timeout")); } }, timeoutMs);
    });
  }
...
async function openPage(compUrl) {
  ...
  const evalJs = async (expr, awaitPromise = false) => {
    const r = await cdp.send("Runtime.evaluate",
      { expression: expr, returnByValue: true, awaitPromise }, sessionId, 60_000);
  ...
  if (!(await evalJs("!!window.__player"))) throw new Error("HF runtime player never appeared — is this a preview comp URL?");
...
const fmtV = (v, axis) => v === null ? "n/a" : axis === "z" ? v.toFixed(3) + " es/s" : v.toFixed(0) + " px/s";

// ---------- verify ----------
async function verify() {
  const ledgerPath = flag("ledger", "ledger.json");
  const ledger = JSON.parse(readFileSync(ledgerPath, "utf8"));
  const fps = ledger.fps || FPS, dt = 1 / fps;
  ...
  for (const seam of ledger.seams) {
    ...
    const tA1 = Math.max(0, cut - 0.1), tA2 = Math.max(0, cut - dt);
    const tB1 = cut + dt, tB2 = cut + 0.1;
    const sels = [seam.exit?.selector, seam.entry?.selector, seam.carrier?.out, seam.carrier?.in]
      .filter(Boolean);
    const S = {};
    ...
    if (type === "cut") {
      const ex = seam.exit, en = seam.entry;
      // 0 — ledger row itself
      if (ex.axis !== en.axis || ex.dir !== en.dir)
        add("ledger", "FAIL", `exit ${ex.axis}${ex.dir > 0 ? "+" : "-"} vs entry ${en.axis}${en.dir > 0 ? "+" : "-"} — mirrored/mixed vector in the PLAN`);
      ...
      for (const [side, cfg, m1, m2, t1, t2] of [
        ...
        if (!m1 || !m2) { add(side, "FAIL", `selector ${cfg.selector} not found`); continue; }
        const v = velocity(m1, m2, t2 - t1, cfg.axis);
        const moving = Math.abs(v) >= eps(cfg.axis);
        const vizOk = side === "exit" ? visible(m1) : visible(m2);
        if (!vizOk) add(side + "-visible", "FAIL", `${cfg.selector} not visible in its window (op ${(side === "exit" ? m1 : m2)?.op?.toFixed(2)})`);
        if (!moving) add(side + "-moving", "FAIL", `${cfg.selector} static at the cut (${fmtV(v, cfg.axis)}) — ${side === "exit" ? "exit settled before the boundary" : "entry starts from rest"}`);
        else if (sgn(v) !== cfg.dir) add(side + "-direction", "FAIL", `${cfg.selector} moving ${fmtV(v, cfg.axis)} — opposite of ledger dir ${cfg.dir > 0 ? "+" : "-"}${cfg.axis === "z" ? " (mirrored zoom)" : ""}`);
        else add(side + "-vector", "PASS", `${fmtV(v, cfg.axis)} ${cfg.axis}${cfg.dir > 0 ? "+" : "-"}`);
        if (side === "exit") seam.__vExit = v; else seam.__vEntry = v;
      }
      ...
      // zero overlap
      const enPre = S[tA2][en.selector], exPost = S[tB1][ex.selector];
      if (visible(enPre)) add("zero-overlap", "FAIL", `incoming ${en.selector} already visible at cut-1f (op ${enPre.op.toFixed(2)}) while outgoing still on screen — reads as a dissolve`);
      else if (visible(exPost)) add("zero-overlap", "FAIL", `outgoing ${ex.selector} still visible at cut+1f (op ${exPost.op.toFixed(2)})`);
      ...
      if (en.axis === "z") {
        ...
        for (const e2 of s2) {
          ...
          if (Math.abs(vs) >= EPS_Z && sgn(vs) !== en.dir) offenders.push(`${e2.sel} (${vs.toFixed(3)} es/s)`);
        }
        if (offenders.length)
          add("z-sign-scan", "FAIL", `elements scaling AGAINST the seam's Z sign in the entry window: ${offenders.slice(0, 5).join(", ")}${offenders.length > 5 ? ` +${offenders.length - 5} more` : ""}`);
    ...
    if (seam.carrier) {
      const out = S[tA2][seam.carrier.out], inn = S[tB1][seam.carrier.in];
      if (!out || !inn) add("carrier", "FAIL", "carrier selector not found");
      else {
        const dx = Math.abs(out.cx - inn.cx), dy = Math.abs(out.cy - inn.cy);
        const ds = Math.abs(out.w - inn.w) / Math.max(out.w, 1);
        if (dx > CARRIER_POS_TOL || dy > CARRIER_POS_TOL)
          add("carrier-position", "FAIL", `center off by ${dx.toFixed(0)},${dy.toFixed(0)}px across the cut`);
        else if (ds > CARRIER_SIZE_TOL)
          add("carrier-size", "FAIL", `size differs ${(ds * 100).toFixed(1)}% across the cut (ancestor scale?)`);
        else add("carrier", "PASS", `Δpos ${dx.toFixed(1)},${dy.toFixed(1)}px Δsize ${(ds * 100).toFixed(1)}%`);
      }
    }
...
async function probe() {
  ...
  const movers = (a, b, span) => join(a, b).map(({ e1, e2 }) => ({
    sel: e2.sel,
    vx: (e2.cx - e1.cx) / span, vy: (e2.cy - e1.cy) / span, vs: (e2.es - e1.es) / span,
    op1: e1.op, op2: e2.op, w: e2.w, h: e2.h,
  })).filter((m) => (m.op1 > VIS || m.op2 > VIS) &&
      (Math.abs(m.vx) > EPS_XY || Math.abs(m.vy) > EPS_XY || Math.abs(m.vs) > EPS_Z || Math.abs(m.op2 - m.op1) > 0.1))
    .sort((x, y) => (Math.abs(y.vx) + Math.abs(y.vy) + Math.abs(y.vs) * 800) - (Math.abs(x.vx) + Math.abs(x.vy) + Math.abs(x.vs) * 800))
    .slice(0, 14);
  const fmt = (m) => `  ${m.sel.padEnd(44)} vx ${m.vx.toFixed(0).padStart(6)}  vy ${m.vy.toFixed(0).padStart(6)}  vscale ${m.vs.toFixed(3).padStart(7)}  op ${m.op1.toFixed(2)}→${m.op2.toFixed(2)}  (${m.w.toFixed(0)}×${m.h.toFixed(0)})`;
  ...
  if (mode === "probe") {
    ...
    if (has("json")) {
      ...
      for (const r of results) {
        ...
        fails += bad; warns += r.rows.filter((x) => x.status === "WARN").length;
