...
function tintCols(o) {
  if (o.tintFrom && (o.tintMixK ?? 1) < 1) {   // mid mood change: blend the old tint into the new on THIS character's palette
    const A = tintCols({ ...o, ...o.tintFrom, tintFrom: null }), B = tintCols({ ...o, tintFrom: null }), k = clamp(o.tintMixK);
    return { col: mixCol(A.col, B.col, k), dk: mixCol(A.dk, B.dk, k), lt: mixCol(A.lt, B.lt, k) };
  }
...
//           legLift [4] (0..1 per leg, tucks it up), legSwing [4] (u per leg, + = toward the heading), walkK (0..1 stride weight)
...
function clawd(x, y, u, o = {}) {
  ...
  if (!o.noLegs) V.legs.forEach(([lx, isFar], i) => {
    ...
    if (o.walkK != null) { const k = clamp(o.walkK); sx *= k; h = 2.2 + (h - 2.2) * k; }   // blend a stride out/in
    if (o.legLift) h *= 1 - .85 * clamp(o.legLift[i] || 0);   // 0..1 per leg: tuck it up (a hurt paw, sitting)
    if (o.legSwing) sx += o.legSwing[i] || 0;                  // u per leg, + = toward the heading (a kick)
...
const EMO = {
  ...
  ouch:       { eyes: 'squeeze', mouth: 'wail', tint: 'pale', tintK: .5, emote: '!!', take: 1.2, body: t => ({ dx: .1 * Math.sin(t * TAU * 16), sq: -.04, aL: 1.1 + .3 * Math.sin(t * TAU * 7), aR: .9 - .3 * Math.sin(t * TAU * 7 + 1) }) },
...
function emotions(t, keys, o = {}) {
  ...
  if (prev && age < .5) {
    ...
    // the colour cross-fade is handed to the character as tint parameters (tintCols resolves it on its own palette),
    // so the same mood timeline works on any character, not just a terracotta one
    cur.tintFrom = { tint: prev.tint, tintK: prev.tintK }; cur.tintMixK = kc;
