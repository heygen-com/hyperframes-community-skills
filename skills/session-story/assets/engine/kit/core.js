...
// Use sparingly. Clawd's emotes are painted and never need these.
...
function drawLetters(c) {
  for (const L of LETTERS) {
    ...
    c.save(); c.translate(L.x, L.y); c.rotate(L.rot || 0); if (L.mat) c.transform(L.mat[0], L.mat[1], L.mat[2], L.mat[3], 0, 0); c.scale(k * (L.sx ?? 1), k * (L.sy ?? 1)); c.globalAlpha = L.alpha ?? 1;   // mat: lay text on a 3D face; sx: foreshorten
...
async function setup() {
  ...
  window.ready = true;
  if (window.__clawdReady) window.__clawdReady();   // HyperFrames bridge (kit/hf-bridge.js): frames can paint now
  else if (!location.search.includes('render')) devUI();
}
function draw() {
...
// HyperFrames bridge: paint time t into #out without encoding it (the renderer screenshots the page).
